...
SDRnullFile <- "C:/Users/norab/Master/data/simulation/simNull/simNull_5.csv"
...
keep_genes_super_SDR_50 <- SUPERdata[SUPERdata$SDR < 0.62,]
```

# Save gene list
```{r}
genes = keep_genes_super$gene
genes50 = keep_genes_super_SDR_50$gene
...
lapply(genes, write, "C:/Users/norab/Master/data/SDR/SDR1005_significantGenesFullname.csv", append=TRUE)
lapply(genes3, write, "C:/Users/norab/Master/data/SDR/SDR1005_significantGenesFullname_ENS.csv", append=TRUE)
lapply(genes50lowest, write, "C:/Users/norab/Master/data/SDR/50LowestSignificantGenes_ENS.csv", append=TRUE)


lapply(SUPERdata, write, "C:/Users/norab/Master/data/SDR/SDR1005_significantGenesSUPER.csv", append=TRUE)
write.table(SUBdata, file = "C:/Users/norab/Master/data/SDR/SDR1005_significantGenesSUB.csv", quote = FALSE, sep = ",", row.names = FALSE)
...
SDR_RBL1 <- sum(filter(keep_genes_super, gene == "ENSG00000080839___RBL1"))
...
SDRsuper[SDRsuper$gene ==  "ENSG00000080839___RBL1",]
...
```{r}
SDRnullFile_dummy <- "C:/Users/norab/Master/data/simulation/simNull/simNull_0.csv"


calcAndSave_simNull_pval <- function(SDRnullFile, pvalSaveFile) {
  
  # Read data
  SDRnullData <- read.csv(SDRnullFile)
  col = colnames(SDRnullData)
  data = unlist(strsplit(col, '_'))
  #G = strtoi(gsub("G.", "", data[1]))
  #P = strtoi(gsub("P.", "", data[2]))
  SDR = as.double(gsub("SDR.", "", data[3]))
  uniq_vals = length(unique(SDRnullData[,1]))
  
  
  P = ecdf(SDRnullData[,1])    # The function giving empirical CDF of SDRnull_values
  p_val = P(SDR)                # Returns empirical CDF at SDRtrue, ie. calculated SDR for populations
                                   # Save p val for next step - multiple testing correcting
  #plot(P)                         # Plot the shit (dont do for all, make an example or two...
  # Save p val to file
  cat(c(colnames(SDRnullData), uniq_vals, p_val), file = pvalSaveFile, sep = ",", append = TRUE)
  cat('\n', file = pvalSaveFile, append=TRUE)
}
```
...
cat(c("data", "unique_pvals", "pval"), file = simNull_pvals_file, append=TRUE)
...
# Read data
p_vals <- read.csv("C:/Users/norab/Master/data/simulation/simNull_pvals.csv", sep=",", col.names = c("data", "unique_pvals", "pval"))
