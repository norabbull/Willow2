...
prob = 0.05
var_p = prob * (1 - prob)
num_perm_samples = 73      # 70 = empirical p val = 0.1
...
calcAndSave_simNull_pval <- function(SDRnullFile, pvalSaveFile) {
  ...
  uniq_pvals = length(unique(SDRnullData[,1]))
  ...
  # plot(P)                         # Plot the shit (dont do for all, make an example or two...
  # Save p val to file
  #SDR <- as.character(SDR)
  cat(c(G, perm, SDR, uniq_pvals, p_val), file = pvalSaveFile, sep = ",", append = TRUE)
  cat('\n', file = pvalSaveFile, append=TRUE)
}
...
cat(c("group_size", "perm", "SDR", "n_uniq_pvals", "pval"), file = simNull_pvals_file, append=TRUE)
