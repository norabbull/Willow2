...
```{r}
library(ape)
tree<-read.tree('C:/Users/norab/Master/data/real_tree_data/ninja_trees/ENSG00000092841___CASPA_HUMAN__full_aln.tre')
tree<-read.tree('C:/Users/norab/Master/data/real_tree_data/ninja_trees/ENSG00000003400___CASPA_HUMAN__full_aln.tre')


pairDist <- cophenetic(tree)
path <- "C:/Users/norab/Master/thesis_data/inspect_single_trees/ENSG00000003400___CASPA_HUMAN_phydist.csv"   # SAVE FILES TO THIS DIRECTORY
  name <-  gsub("^.*ninja_trees/", "", file)
  name <- gsub("_HUMAN__full_aln.tre$", "", name)
  filename <- paste0(path, name)
  filename <- paste0(filename, "___CopD.csv")
  print("File: ", filename)
  write.csv(pairDist, file=path, row.names=TRUE, col.names=TRUE, sep = ",", quote = F)
